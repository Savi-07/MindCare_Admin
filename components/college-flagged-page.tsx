...
export function CollegeFlaggedPage() {
  const { userCollege } = useAuth();
  const [searchTerm, setSearchTerm] = useState("");
  const [riskFilter, setRiskFilter] = useState("all");

  // Filter flagged users for the specific college
  ...
  const flaggedUsers = collegeUsers.filter(
    (user) => user.riskLevel === "medium" || user.riskLevel === "high"
  );
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 text-lg">
              Monitor and manage students requiring immediate attention at{" "}
              {userCollege}
            </p>
          </div>
        </div>
        ...
        <Card className="border-l-4 border-l-orange-500 hover:shadow-lg transition-all duration-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {Math.round((flaggedUsers.length / collegeUsers.length) * 100)}%
              of college students
            </p>
          </CardContent>
        </Card>
      ...
      <Card className="border border-red-200 dark:border-red-800 bg-gradient-to-br from-red-50/50 to-orange-50/50 dark:from-red-900/20 dark:to-orange-900/20">
        <CardHeader className="bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/30 dark:to-orange-900/30 rounded-t-lg">
          ...
          <p className="text-sm text-red-600 dark:text-red-400">
            Search and filter flagged students efficiently
          </p>
        </CardHeader>
      ...
      <Card className="border border-slate-200 dark:border-slate-700 bg-gradient-to-br from-slate-50/50 to-gray-50/50 dark:from-slate-900/20 dark:to-gray-900/20">
        <CardHeader className="bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-900/30 dark:to-gray-900/30 rounded-t-lg">
          <CardTitle className="flex items-center gap-3 text-slate-800 dark:text-slate-200">
            ...
            <span className="text-lg font-semibold">Flagged Students</span>
            <Badge
              variant="secondary"
              className="ml-2 bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-200"
            >
              {filteredUsers.length} cases
            </Badge>
          </CardTitle>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Detailed risk assessment and student information
          </p>
        </CardHeader>
        <CardContent className="p-6">
          <div className="space-y-6">
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                <div
                  key={user.id}
                  className={`border border-slate-200 dark:border-slate-700 rounded-xl p-6 space-y-6 bg-white dark:bg-slate-800 hover:shadow-lg transition-all duration-200 border-l-4 ${getRiskColor(
                    user.riskLevel
                  )}`}
                >
                  {/* User Header */}
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-400 dark:text-slate-500 mb-4">
                  No flagged students match your current filters
                </p>
                <Button
                  variant="outline"
